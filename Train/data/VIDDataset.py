...
class VIDDataset(Dataset):
    ...
    def __getitem__(self, rand_vid):
        ...
        img_x_z = self.z_transforms(img_x)
        img_x = self.x_transforms(img_x)

        # return img_z, img_x_z, img_x
        return img_z, img_x
...
class RASNet_VIDDataset(Dataset):
    ...
    def __getitem__(self, rand_vid):
        ...
        penalty = torch.from_numpy(np.exp(-np.abs(rand_z-rand_x)/100))


        return img_z, img_x, penalty
